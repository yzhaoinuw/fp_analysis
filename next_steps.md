...
5. [x] Save and undo modified event annotations.
   ...
   - Acceptance gate: accepted when the user advanced to the next checklist item; users can delete event timestamps, undo the most recent edit, save a MAT copy, reload it, and see the saved event timestamp state.

6. [x] Positive-integer baseline and analysis window inputs.
   ...
   - Keep wide-window perievent x axes readable by adapting tick density, label font size, and rotation across trace, mean, normalized-trace, and heatmap panels.
   - Acceptance gate: accepted after the user confirmed invalid values cannot launch analysis, valid integer values do, and adaptive plot ticks remain readable with a wide window.

7. [x] Perievent heatmap row dividers.
   - Add solid black dividers between event-occurrence rows in the perievent heatmap generated by `Perievent_Plots.plot_perievent_heatmaps`.
   - Progressively reduce divider width and opacity as event count grows.
   - Label every row on low-count heatmaps and sample at most about 15 event-index labels on dense heatmaps instead of rendering blank tick labels.
